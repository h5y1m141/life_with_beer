class Picture < ActiveRecord::Base
  ...
  def self.image_source(source)
    picture_image = self.new
    picture_image.image = self.base64_conversion(source)
    picture_image
  end

  def self.base64_conversion(uri_str, filename = 'base64')
    image_data = self.split_base64(uri_str)
